...
import { ZakkBotAnalytics, QueryAnalytics, UserAnalytics } from "./usage/types";
...
export const useZakkBotAnalytics = (timeRange: DateRangePickerValue) => {
  const url = buildApiPath("/api/analytics/admin/zakkbot", {
    start: convertDateToStartOfDay(timeRange.from)?.toISOString(),
    end: convertDateToEndOfDay(timeRange.to)?.toISOString(),
  });
  const swrResponse = useSWR<ZakkBotAnalytics[]>(url, errorHandlingFetcher); // TODO

  return {
    ...swrResponse,
    refreshZakkBotAnalytics: () => mutate(url),
  };
};
