...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        for file_info in zip_file.infolist():
            ...
            ) or file_info.filename == ZAKK_METADATA_FILENAME:
                continue
...
def read_text_file(
    ...
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_zakk_metadata:
            potential_meta = _extract_zakk_metadata(line)
...
def docx_to_text_and_images(
    ...
    try:
        ...
        text_content_raw, _ = read_text_file(
            file, encoding=encoding, ignore_zakk_metadata=False
        )
