...
# Chat sessions do not include zakkbot flows
def get_chat_sessions_by_user(
    user_id: UUID | None,
    deleted: bool | None,
    db_session: Session,
    include_zakkbot_flows: bool = False,
    limit: int = 50,
) -> list[ChatSession]:
    stmt = select(ChatSession).where(ChatSession.user_id == user_id)

    if not include_zakkbot_flows:
        stmt = stmt.where(ChatSession.zakkbot_flow.is_(False))
...
def delete_all_chat_sessions_for_user(
    ...
    query = db_session.query(ChatSession).filter(
        ChatSession.user_id == user_id, ChatSession.zakkbot_flow.is_(False)
    )
