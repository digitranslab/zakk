from zakk.configs.app_configs import DISABLE_USER_KNOWLEDGE
from zakk.configs.app_configs import ZAKK_QUERY_HISTORY_TYPE
from zakk.configs.constants import KV_SETTINGS_KEY
...
def load_settings() -> Settings:
    ...
    try:
        value = redis_client.get(ZakkRedisLocks.ANONYMOUS_USER_ENABLED)
        if value is not None:
            ...
            redis_client.set(ZakkRedisLocks.ANONYMOUS_USER_ENABLED, "0")
    except Exception as e:
    ...
    settings.query_history_type = ZAKK_QUERY_HISTORY_TYPE
...
def store_settings(settings: Settings) -> None:
    ...
    if settings.anonymous_user_enabled is not None:
        redis_client.set(
            ZakkRedisLocks.ANONYMOUS_USER_ENABLED,
