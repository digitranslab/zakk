...
def extract_zip_metadata(zf: zipfile.ZipFile) -> dict[str, Any]:
    zip_metadata = {}
    try:
        metadata_file_info = zf.getinfo(ZAKK_METADATA_FILENAME)
        with zf.open(metadata_file_info, "r") as metadata_file:
            try:
                ...
                logger.warning(f"Unable to load {ZAKK_METADATA_FILENAME}: {e}")
                # should fail loudly here to let users know that their metadata
                # file is not valid JSON
                raise HTTPException(
                    status_code=400,
                    detail=f"Unable to load {ZAKK_METADATA_FILENAME}: {e}",
                )
    except KeyError:
        logger.info(f"No {ZAKK_METADATA_FILENAME} file")
    return zip_metadata
