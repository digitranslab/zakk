...
def remove_zakk_bot_tag(tenant_id: str, message_str: str, client: WebClient) -> str:
    bot_token_user_id, _ = get_zakk_bot_auth_ids(tenant_id, web_client=client)
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            if is_zakk_bot_response:
                ...
                if not blocks:
                    logger.warning(f"ZakkBot response has no blocks: {reply}")
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the ZakkBot answer format, if there is a change to how we respond,
        ...
        message = remove_zakk_bot_tag(tenant_id, message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            zakk_user = get_user_by_email(email=sender_email, db_session=db_session)
