...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_token_user_id, bot_token_bot_id = get_zakk_bot_auth_ids(
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_zakk_bot_tag(tenant_id, msg, client=client.web_client)
        ...
        if event_type == "message":
            ...
            if bot_token_user_id and bot_token_user_id in event_user:
                is_zakk_bot_msg = True

            if bot_token_bot_id and bot_token_bot_id in event_bot_id:
                is_zakk_bot_msg = True

            # ZakkBot should never respond to itself
            if is_zakk_bot_msg:
                logger.info("Ignoring message from ZakkBot (self-message)")
                return False

            # DMs with the bot don't pick up the @ZakkBot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If ZakkBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received ZakkBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to ZakkBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        bot_token_user_id, _ = get_zakk_bot_auth_ids(tenant_id, client.web_client)
        message_ts = event.get("ts")
        thread_ts = event.get("thread_ts")
        ...
        if tagged:
            logger.debug("User tagged ZakkBot")
...
def _get_socket_client(
    ...
    # https://docs.digi-trans.org/slack_bot_setup
