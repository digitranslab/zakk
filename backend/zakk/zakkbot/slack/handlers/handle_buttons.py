...
def handle_publish_ephemeral_message_button(
    ...
    # Obtain zakk_user and chat_message information
    if not chat_message_id:
        raise ValueError("Missing chat_message_id in the payload")

    with get_session_with_current_tenant() as db_session:
        zakk_user = get_user_by_email(user_email, db_session)
        if not zakk_user:
            raise ValueError("Cannot determine zakk_user_id from email in payload")
        try:
            chat_message = get_chat_message(chat_message_id, zakk_user.id, db_session)
        except ValueError:
        ...
        zakk_bot_answer = ChatZakkBotResponse(
