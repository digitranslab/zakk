...
from zakk.configs.zakkbot_configs import ZAKK_BOT_FEEDBACK_REMINDER
...
def schedule_feedback_reminder(
    ...
    if not ZAKK_BOT_FEEDBACK_REMINDER:
    ...
    future = now + datetime.timedelta(minutes=ZAKK_BOT_FEEDBACK_REMINDER)
...
def handle_message(
    ...
    if slack_channel_config.channel_config.get("disabled"):
        logger.info("Skipping message: ZakkBot is disabled for this channel")
        return False
    ...
    if respond_tag_only and not bypass_filters and not is_bot_dm:
        logger.info("Skipping message: ZakkBot only responds to tags in this channel")
        return False
    ...
    # If configured to respond to team members only, then cannot be used with a /ZakkBot command
    ...
    if send_to and is_bot_msg:
        if sender_id:
            respond_in_thread_or_channel(
                ...
                text="The ZakkBot slash command is not enabled for this channel",
                thread_ts=None,
            )
