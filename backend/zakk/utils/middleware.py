...
def add_zakk_request_id_middleware(
...
def make_randomized_zakk_request_id(prefix: str) -> str:
    """generates a randomized request id"""

    hash_input = str(uuid.uuid4())
    return _make_zakk_request_id(prefix, hash_input)


def make_structured_zakk_request_id(prefix: str, request_url: str) -> str:
    """Not used yet, but could be in the future!"""
    hash_input = f"{request_url}:{datetime.now(timezone.utc)}"
    return _make_zakk_request_id(prefix, hash_input)


def _make_zakk_request_id(prefix: str, hash_input: str) -> str:
    ...
    zakk_request_id = f"{prefix}:{hash_str}"
    return zakk_request_id
