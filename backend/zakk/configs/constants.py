...
ZAKK_DEFAULT_APPLICATION_NAME = "Zakk"
ZAKK_SLACK_URL = "https://join.slack.com/t/digitranslab/shared_invite/zt-2twesxdr6-5iQitKZQpgq~hYIZ~dv3KA"
ZAKK_EMAILABLE_LOGO_MAX_DIM = 512
...
TENANT_ID_COOKIE_NAME = "zakk_tid"  # tenant id - for workaround cases
ANONYMOUS_USER_COOKIE_NAME = "zakk_anonymous_user"

NO_AUTH_USER_ID = "__no_auth_user__"
NO_AUTH_USER_EMAIL = "anonymous@digi-trans.org"
...
ZAKK_METADATA_FILENAME = ".zakk_metadata.json"

# Messages
DISABLED_GEN_AI_MSG = (
    "Your System Admin has disabled the Generative AI functionalities of Zakk.\n"
    "Please contact them if you wish to have this enabled.\n"
    "You can still use Zakk as a search engine."
)
...
KV_SETTINGS_KEY = "zakk_settings"
KV_CUSTOMER_UUID_KEY = "customer_uuid"
KV_INSTANCE_DOMAIN_KEY = "instance_domain"
KV_ENTERPRISE_SETTINGS_KEY = "zakk_enterprise_settings"
...
class MilestoneRecordType(str, Enum):
    ...
    CREATED_ZAKK_BOT = "created_zakk_bot"
...
class ZakkRedisConstants:
    ACTIVE_FENCES = "active_fences"


class ZakkCeleryPriority(int, Enum):
    HIGHEST = 0
    HIGH = auto()
    MEDIUM = auto()
    LOW = auto()
...
ZAKK_CLOUD_CELERY_TASK_PREFIX = "cloud"

# the tenant id we use for system level redis operations
ZAKK_CLOUD_TENANT_ID = "cloud"

# the redis namespace for runtime variables
ZAKK_CLOUD_REDIS_RUNTIME = "runtime"
CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT = 600


class ZakkCeleryTask:
    DEFAULT = "celery"

    CLOUD_BEAT_TASK_GENERATOR = f"{ZAKK_CLOUD_CELERY_TASK_PREFIX}_generate_beat_tasks"
    CLOUD_MONITOR_ALEMBIC = f"{ZAKK_CLOUD_CELERY_TASK_PREFIX}_monitor_alembic"
    CLOUD_MONITOR_CELERY_QUEUES = (
        f"{ZAKK_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_queues"
    )
    CLOUD_CHECK_AVAILABLE_TENANTS = (
        f"{ZAKK_CLOUD_CELERY_TASK_PREFIX}_check_available_tenants"
    )
    CLOUD_MONITOR_CELERY_PIDBOX = (
        f"{ZAKK_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_pidbox"
    )
...
ZAKK_CELERY_BEAT_HEARTBEAT_KEY = "zakk:celery:beat:heartbeat"
