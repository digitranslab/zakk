...
from ee.zakk.server.enterprise_settings.store import get_logo_filename
from ee.zakk.server.enterprise_settings.store import get_logotype_filename
from ee.zakk.server.enterprise_settings.store import load_analytics_script
from ee.zakk.server.enterprise_settings.store import load_settings
from ee.zakk.server.enterprise_settings.store import store_analytics_script
from ee.zakk.server.enterprise_settings.store import store_settings
...
def fetch_logo_helper(db_session: Session) -> Response:
    try:
        ...
        zakk_file = file_store.get_file_with_mime_type(get_logo_filename())
        if not zakk_file:
            raise ValueError("get_zakk_file returned None!")
    except Exception:
        ...
        return Response(content=zakk_file.data, media_type=zakk_file.mime_type)


def fetch_logotype_helper(db_session: Session) -> Response:
    try:
        file_store = get_default_file_store()
        zakk_file = file_store.get_file_with_mime_type(get_logotype_filename())
        if not zakk_file:
            raise ValueError("get_zakk_file returned None!")
    except Exception:
        raise HTTPException(
