...
def _drive_folder_to_zakk_group(
    folder: FolderInfo,
...
def _drive_member_map_to_zakk_groups(
    drive_id_to_members_map: dict[str, tuple[set[str], set[str]]],
    group_email_to_member_emails_map: dict[str, list[str]],
...
def _google_group_to_zakk_group(
    admin_service: AdminService,
    group_email: str,
) -> ExternalUserGroup:
    """
...
def gdrive_group_sync(
    ...
    for group_email in all_group_emails:
        ...
        group_email_to_member_emails_map[group_email] = zakk_group.user_emails
        yield zakk_group

    # Each drive is a group, yield those
    for zakk_group in _drive_member_map_to_zakk_groups(
    ...
    for folder in folder_info:
        yield _drive_folder_to_zakk_group(folder, group_email_to_member_emails_map)
