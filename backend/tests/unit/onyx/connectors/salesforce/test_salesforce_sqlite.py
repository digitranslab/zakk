...
def _create_csv_with_example_data(sf_db: ZakkSalesforceSQLite) -> None:
    """
...
def _test_query(sf_db: ZakkSalesforceSQLite) -> None:
    """
...
def _test_upsert(sf_db: ZakkSalesforceSQLite) -> None:
    """
...
def _test_relationships(sf_db: ZakkSalesforceSQLite) -> None:
    """
...
def _test_account_with_children(sf_db: ZakkSalesforceSQLite) -> None:
    """
...
def _test_relationship_updates(sf_db: ZakkSalesforceSQLite) -> None:
    """
...
def _test_get_affected_parent_ids(sf_db: ZakkSalesforceSQLite) -> None:
    """
...
def test_salesforce_sqlite() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        sf_db = ZakkSalesforceSQLite(filename)
        sf_db.connect()
        sf_db.apply_schema()
...
def test_salesforce_bulk_retrieve() -> None:
    ...
    # zakk_sf_type = ZakkSalesforceType("Contact", sf_client)
...
def test_normalize_record() -> None:
    ...
    with open(current_dir / "test_account.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            normalized_record, parent_ids = ZakkSalesforceSQLite.normalize_record(row)
...
def test_salesforce_connector_single() -> None:
    ...
    # zakk_parent_sf_type = ZakkSalesforceType(parent_type, sf_client)
    ...
    for parent_type_working in parent_types:
        ...
        for child_type, child_relationship in child_types_working.items():
            # zakk_sf_type = ZakkSalesforceType(child_type, sf_client)
    ...
    normalized_record, _ = ZakkSalesforceSQLite.normalize_record(record)
