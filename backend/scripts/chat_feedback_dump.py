...
def create_new_chat_session(zakk_url: str, api_key: str | None) -> int:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None
    session_endpoint = zakk_url + "/api/chat/create-chat-session"
...
def manage_users(zakk_url: str, headers: dict[str, str] | None) -> AllUsersResponse:
    endpoint = zakk_url + "/manage/users"
...
def get_chat_sessions(
    zakk_url: str, headers: dict[str, str] | None, user_id: UUID
) -> ChatSessionsResponse:
    endpoint = zakk_url + "/admin/chat-sessions"
...
def get_session_history(
    zakk_url: str, headers: dict[str, str] | None, session_id: UUID
) -> ChatSessionSnapshot:
    endpoint = zakk_url + f"/admin/chat-session-history/{session_id}"
...
def process_all_chat_feedback(zakk_url: str, api_key: str | None) -> None:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None

    all_users = manage_users(zakk_url, headers)
    if not all_users:
...
if __name__ == "__main__":
    ...
    process_all_chat_feedback(zakk_url=args.url, api_key=args.api_key)
