...
```
PYTHONPATH=. python scripts/debugging/zakk_vespa_schemas.py
```
...
def generate_document_entries() -> str:
    ...
    for model in SUPPORTED_EMBEDDING_MODELS:
        ...
        # Add alt index
        document_entries.append(
            f'            <document type="{model.index_name}__zakk_alt_index" mode="index" />'
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="zakk/document_index/vespa/app_config/schemas/zakk_chunk.sd.jinja",
    )
    parser.add_argument(
    ...
    for model in SUPPORTED_EMBEDDING_MODELS:
        ...
        write_schema(
            model.index_name + "__zakk_alt_index",
            model.dim,
            model.embedding_precision,
            template,
            output_path,
