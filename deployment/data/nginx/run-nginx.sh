...
export ZAKK_BACKEND_API_HOST="${ZAKK_BACKEND_API_HOST:-api_server}"
export ZAKK_WEB_SERVER_HOST="${ZAKK_WEB_SERVER_HOST:-web_server}"

export SSL_CERT_FILE_NAME="${SSL_CERT_FILE_NAME:-ssl.crt}"
export SSL_CERT_KEY_FILE_NAME="${SSL_CERT_KEY_FILE_NAME:-ssl.key}"

echo "Using API server host: $ZAKK_BACKEND_API_HOST"
echo "Using web server host: $ZAKK_WEB_SERVER_HOST"

envsubst '$DOMAIN $SSL_CERT_FILE_NAME $SSL_CERT_KEY_FILE_NAME $ZAKK_BACKEND_API_HOST $ZAKK_WEB_SERVER_HOST' < "/etc/nginx/conf.d/$1" > /etc/nginx/conf.d/app.conf
...
echo
echo "docker logs zakk-stack-api_server-1"
echo

while true; do
  # Use curl to send a request and capture the HTTP status code
  status_code=$(curl -o /dev/null -s -w "%{http_code}\n" "http://${ZAKK_BACKEND_API_HOST}:8080/health")
