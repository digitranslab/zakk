...
STACK_NAMES=(
  "${ENVIRONMENT}-zakk-nginx-service"
  "${ENVIRONMENT}-zakk-web-server-service"
  "${ENVIRONMENT}-zakk-backend-background-server-service"
  "${ENVIRONMENT}-zakk-backend-api-server-service"
  "${ENVIRONMENT}-zakk-model-server-inference-service"
  "${ENVIRONMENT}-zakk-model-server-indexing-service"
  "${ENVIRONMENT}-zakk-vespaengine-service"
  "${ENVIRONMENT}-zakk-redis-service"
  "${ENVIRONMENT}-zakk-postgres-service"
  "${ENVIRONMENT}-zakk-cluster"
  "${ENVIRONMENT}-zakk-acm"
  "${ENVIRONMENT}-zakk-efs"
  )

delete_stack() {
  local stack_name=$1

  if [ "$stack_name" == "${ENVIRONMENT}-zakk-cluster" ]; then
      echo "Removing all objects and directories from the zakk config s3 bucket."
